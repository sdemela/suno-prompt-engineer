...
// api/save-email.js
// FIX #1: two explicit flows:
//   sign-only (no email) → genera solo sig, no Redis, no Resend
//   save+email (email valida) → persiste mapping + invia email + ritorna sig
...
export default async function handler(req, res) {
  ...
  const ip = getTrustedIp(req); // FIX #2

  // Rate limit UUID cooldown
  ...
  try {
    ...
    if (!emailRes.ok) {
      ...
      // Non bloccare l'utente se l'email fallisce
    }
